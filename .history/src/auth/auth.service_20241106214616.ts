...
export class AuthService {
  ...
  // ======================= LEARNER========================
  async signUp_learner(createLearnerDto: CreateLearnerDto, res: Response) {
    const existingLearner = await this.learnerService.findLearnerByEmail(
    ...
    const newLearner = await this.learnerService.create({
      ...createLearnerDto,
      hashed_password,
    });
    ...
    const hashed_refresh_token = await bcrypt.hash(tokens.refresh_token, 7);
    const activation_link = uuid.v4(); // Assuming you have a UUID generation utility

    const updatedLearner = await this.learnerService.update(newLearner.id, {
      hashed_refresh_token,
      activation_link,
    });
    ...
    // Assuming mailService is set up for sending emails
    try {
      await this.mailService.sendMailLearner(updatedLearner, activation_link);
    } catch (error) {
    ...
    return {
      message:
        'Learner registered successfully. Please verify via the link sent to your email.',
  ...
  async activateLearner(link: string, res: Response) {
    const learner = await this.learnerService.findByActivationLink(link);
    if (!learner) {
      return res
        .status(400)
        .json({ message: 'Learner not found or invalid activation link!' });
    }
    if (learner.is_active) {
      return res.status(400).json({ message: 'Learner is already activated.' });
    }

    learner.is_active = true;
    await this.learnerService.save(learner);
  ...
  async signIn_learner(signInDto: LearnerSignInDto, res: Response) {
    const learner = await this.learnerService.findLearnerByEmail(
      signInDto.email,
    );
    if (!learner || !learner.is_active) {
      throw new UnauthorizedException(
        'Invalid login credentials or account not activated',
      );
    }
    ...
    await this.learnerService.update(learner.id, { hashed_refresh_token });
  ...
  async signOutLearner(refreshToken: string, res: Response) {
    try {
      const payload = await this.jwtService.verifyAsync(refreshToken, {
        secret: process.env.LEARNER_REFRESH_TOKEN_KEY,
      });

      const learner = await this.learnerService.findOne(payload.sub);
      if (!learner || learner.hashed_refresh_token === null) {
        throw new UnauthorizedException(
          'Learner not found or already signed out',
        );
      }
      ...
      await this.learnerService.update(learner.id, {
      ...
      throw new BadRequestException('Failed to sign out learner');
    }
  }
  ...
  //     } catch (error) {
  //       console.log(error);
  //       throw new InternalServerErrorException('Error on sending mail to Learner email');
  //     }
  ...
  //   async learnerSignIn(ownerSignInDto: SignInDto, res: Response) {
  //     const { email, password } = ownerSignInDto;
  //     const learner = await this.learnerModel.findOne({
  //       where: { email },
  //     });
  ...
  //     const validPassword = await bcrypt.compare(password, learner.hashed_password);
  //     if (!validPassword) {
